...
namespace XamList.Tests.Shared
{
    public abstract class BaseTest
    {
        ...
        #region Methods
        protected List<ContactModel> GetTestContacts(List<ContactModel> contactList)
        {
            return contactList.Where(x => x.FirstName.Equals(TestConstants.TestFirstName) &&
                                            x.LastName.Equals(TestConstants.TestLastName) &&
                                            x.PhoneNumber.Equals(TestConstants.TestPhoneNumber)).ToList();
        }

        protected async Task RemoveTestContactsFromRemoteDatabase()
        {
            ...
            var testContactList = GetTestContacts(contactList);
